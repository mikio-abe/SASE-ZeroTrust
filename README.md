...
Both Linux POP nodes required manual installation and configuration of Cloudflare components.
This section documents the full client-side setup for Zero Trust enrollment.

### Installed Components

| Component | Purpose | POP1 | POP2 |
|-----------|---------|------|------|
| cloudflare-warp | SWG traffic proxy via MASQUE (port 7844) | ✓ | ✓ |
| cloudflared | ZTNA tunnel agent / DoH DNS proxy | ✓ | ✓ (DoH active) |
| Cloudflare Root CA | TLS Inspection trust anchor | ✓ | ✓ |
| mdm.xml | Service Token authentication for device enrollment | ✓ | ✓ |

### Package Installation (Debian/Bookworm)

Both POPs are headless Linux (Debian 12) running inside EVE-NG.
No GUI or browser available — all installation performed via CLI.
...
# 3. Install WARP client and cloudflared
apt update && apt install -y cloudflare-warp cloudflared
```

### TLS Certificate Installation

Cloudflare Root CA is required for TLS Inspection (decrypt-inspect-re-encrypt).
Without this certificate, HTTPS connections through SWG will fail with certificate errors.
...
### Device Enrollment (Zero Trust Organization)

Headless Linux devices cannot use browser-based IdP authentication for enrollment.
Service Token authentication was used instead.

**Cloudflare Dashboard Setup:**

| Setting | Location | Value |
|---------|----------|-------|
| Service Token | Access controls → Service credentials | `linux-pop` (1-year expiry) |
| Enrollment Policy | Devices → Management → Device enrollment | Service Auth rule |

**POP-side MDM Configuration:**
...
After placing mdm.xml, WARP service registration connects the device to the eve-lab organization:
...
### DNS Configuration per POP

| POP | DNS Method | DNS Location | How It Works |
|-----|-----------|--------------|--------------|
| CF-POP1 | IPv4 DNS (172.64.36.1) | eve-lab | Source IP identification (106.73.26.0) |
| CF-POP2 | DoH via cloudflared proxy-dns | eve-lab-2 | URL token identification (DoH endpoint) |

POP2 uses cloudflared as a local DNS proxy to route queries through a dedicated DoH endpoint,
enabling separate DNS Location assignment even when both POPs share the same public IP (NAT).

```
# POP2: cloudflared as DoH proxy
cloudflared proxy-dns --upstream https://xx579sxsi4.cloudflare-gateway.com/dns-query
```

### Service Conflict Resolution

cloudflared (port 53) and WARP (port 53) both attempt to control DNS resolution.
Additionally, systemd-resolved occupies port 53 by default on Debian.

**Resolution applied:**

| POP | Active DNS Service | Stopped Service | Reason |
|-----|--------------------|-----------------|--------|
| CF-POP1 | WARP (SWG proxy) | cloudflared proxy-dns, systemd-resolved | WARP handles both DNS and HTTP inspection |
| CF-POP2 | cloudflared proxy-dns | systemd-resolved | DoH required for eve-lab-2 Location identification |
...
| IdP | Provider | Protocol | Purpose |
|-----|----------|----------|---------|
| Auth0 | Okta subsidiary (used because Okta requires business email) | OIDC | Browser-based user authentication |
| Entra ID | Microsoft (Azure AD) | OIDC | Enterprise IdP integration |
| One-time PIN | Cloudflare built-in | Email code | Fallback authentication |

All three IdPs are available as authentication options for ZTNA Access Applications.
IdPs are not tied to specific DNS Locations — they operate at the Access (ZTNA) layer,
independent of Gateway (SWG) DNS policy enforcement.

　**【日本語サマリ】**

CF-POP1/POP2はヘッドレスLinux（Debian 12、EVE-NG内）のため、全てCLIで構築。
GPGキー追加・リポジトリ設定からWARPクライアント・cloudflaredのインストール、
Cloudflare Root CA証明書（TLS Inspection用）の配置、
Service Token認証によるMDMファイル経由のデバイス登録まで、手動で実施。

POP1はWARP経由のIPv4 DNS、POP2はcloudflared DoHプロキシで
それぞれ別のDNS Location（eve-lab / eve-lab-2）に接続。
同一NAT環境でもDoHトークンにより拠点別ポリシー適用とログ分離を実現。

cloudflaredとWARPのポート53競合、およびsystemd-resolvedとの競合を
POP別の役割分担で解決。

IdPはAuth0（Okta傘下・OIDC）、Entra ID（Azure AD・OIDC）、One-time PINの3つを
ZTNA認証オプションとして登録。DNS Locationとは独立したAccess層で動作。
